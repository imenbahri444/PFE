...
import {
  FaSearch, FaEdit, FaTrash, FaIdCard, FaEnvelope, FaPhone,
  FaCalendarAlt, FaCheckCircle, FaTimesCircle, FaArrowLeft,
  FaUserPlus, FaSortAmountDown, FaSortAmountUp, FaChevronDown, FaMapMarkerAlt
} from "react-icons/fa";
...
const INITIAL_USERS = [
  { id:1, name:"Sarah Johnson",  email:"sarah.johnson@company.com",  username:"sarah.j",   matricule:"EMP001", phone:"+33 6 12 34 56 78", role:"Moderator",      department:"Marketing",        position:"Marketing Director",    site:"Leoni Sousse (LSS)",          accessCard:{cardId:"CARD001",cardUID:"A1:B2:C3:D4:E5:F6",issueDate:"2026-01-15",status:"active"}, createdAt:"2026-01-15", lastActive:"2026-02-22", bookingsCount:24, status:"active" },
  { id:2, name:"Michael Chen",   email:"michael.chen@company.com",   username:"michael.c", matricule:"EMP002", phone:"+33 6 23 45 67 89", role:"Developer",      department:"Engineering",      position:"Lead Developer",         site:"Leoni Tunis (LTN)",           accessCard:{cardId:"CARD002",cardUID:"B2:C3:D4:E5:F6:G7",issueDate:"2026-01-16",status:"active"}, createdAt:"2026-01-16", lastActive:"2026-02-23", bookingsCount:18, status:"active" },
  { id:3, name:"Emma Davis",     email:"emma.davis@company.com",     username:"emma.d",    matricule:"EMP003", phone:"+33 6 34 56 78 90", role:"Designer",       department:"Design",           position:"Senior UX Designer",     site:"Leoni Ain Zaghouan 1 (LAZ1)", accessCard:{cardId:"CARD003",cardUID:"C3:D4:E5:F6:G7:H8",issueDate:"2026-01-17",status:"active"}, createdAt:"2026-01-17", lastActive:"2026-02-21", bookingsCount:15, status:"active" },
  { id:4, name:"James Wilson",   email:"james.wilson@company.com",   username:"james.w",   matricule:"EMP004", phone:"+33 6 45 67 89 01", role:"Product Manager",department:"Product",          position:"Senior Product Manager", site:"Leoni Nabeul (LNB)",          accessCard:{cardId:"CARD004",cardUID:"D4:E5:F6:G7:H8:I9",issueDate:"2026-01-18",status:"active"}, createdAt:"2026-01-18", lastActive:"2026-02-22", bookingsCount:12, status:"active" },
  { id:5, name:"Lisa Anderson",  email:"lisa.anderson@company.com",  username:"lisa.a",    matricule:"EMP005", phone:"+33 6 56 78 90 12", role:"QA Engineer",    department:"Quality Assurance",position:"Lead QA Engineer",       site:"Leoni Bizerte (LBZ)",         accessCard:{cardId:"CARD005",cardUID:"E5:F6:G7:H8:I9:J0",issueDate:"2026-01-19",status:"active"}, createdAt:"2026-01-19", lastActive:"2026-02-20", bookingsCount:10, status:"active" },
];

// ─── Helpers ──────────────────────────────────────────────────────────────────
const getInitials    = n => n.split(" ").map(x => x[0]).join("").substring(0,2).toUpperCase();
const getAvatarColors = () => AVATAR_PALETTES[0]; // original: always blue
const fmtDate        = d => d && d !== "Never" ? new Date(d).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"}) : "—";
const loadPersistedUsers = () => {
  try {
    const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY));
    if (parsed?.length) {
      if (parsed[0].site === undefined) { localStorage.removeItem(STORAGE_KEY); return INITIAL_USERS; }
      return parsed;
    }
  } catch {}
...
// ─── CSS (exact original, injected once) ─────────────────────────────────────
const CSS = `
  @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&family=DM+Mono:wght@400;500&display=swap');
  .fmu-root{--accent:#2563eb;--accent-light:#dbeafe;--accent-dark:#1d4ed8;--surface:#fff;--surface-2:#f8fafc;--surface-3:#f1f5f9;--border:#e2e8f0;--border-2:#cbd5e1;--text-1:#0f172a;--text-2:#475569;--text-3:#94a3b8;--green:#10b981;--red:#ef4444;--amber:#f59e0b;--radius:14px;--radius-sm:8px;font-family:'DM Sans',sans-serif;background:#f0f4f8;min-height:100vh;padding:28px 32px;color:var(--text-1);box-sizing:border-box;}
  ...
  .fmu-add-btn{display:flex;align-items:center;gap:8px;padding:10px 18px;background:var(--accent);color:white;border:none;border-radius:var(--radius-sm);font-family:inherit;font-size:0.875rem;font-weight:600;cursor:pointer;transition:background .15s,transform .1s,box-shadow .15s;box-shadow:0 2px 8px rgba(37,99,235,0.35);}
  ...
  .fmu-search{width:100%;padding:10px 12px 10px 34px;border:1.5px solid var(--border);border-radius:var(--radius-sm);font-family:inherit;font-size:0.875rem;background:var(--surface);color:var(--text-1);outline:none;transition:border-color .15s;}
  ...
  .fmu-select{appearance:none;padding:10px 30px 10px 12px;border:1.5px solid var(--border);border-radius:var(--radius-sm);background:var(--surface);font-family:inherit;font-size:0.85rem;color:var(--text-2);cursor:pointer;outline:none;transition:border-color .15s;}
  ...
  .fmu-sort-dir{padding:0 12px;border:none;border-left:1.5px solid var(--border);background:transparent;cursor:pointer;color:var(--text-2);display:flex;align-items:center;transition:background .15s;}
  ...
  .fmu-card{background:var(--surface);border-radius:var(--radius);border:1.5px solid var(--border);padding:20px;cursor:pointer;transition:transform .18s,box-shadow .18s,border-color .18s;position:relative;overflow:hidden;}
  .fmu-card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--c1,#2563eb),var(--c2,#1d4ed8));opacity:0;transition:opacity .18s;}
  ...
  .fmu-avatar{width:48px;height:48px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;font-weight:700;color:white;flex-shrink:0;letter-spacing:-0.02em;}
  .fmu-card-actions{display:flex;gap:6px;}
  .fmu-icon-btn{width:44px;height:44px;border-radius:7px;border:none;background:#e0e7ff;color:#3b5bdb;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:1rem;transition:all .15s;}
  ...
  .fmu-back-btn{display:inline-flex;align-items:center;gap:7px;background:none;border:none;color:var(--text-3);font-family:inherit;font-size:0.85rem;cursor:pointer;padding:0;margin-bottom:22px;transition:color .15s;}
  ...
  .fmu-detail-hero{padding:28px 32px;display:flex;gap:22px;align-items:center;background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);position:relative;overflow:hidden;}
  .fmu-detail-hero::after{content:'';position:absolute;right:-60px;top:-60px;width:200px;height:200px;border-radius:50%;background:rgba(37,99,235,0.15);pointer-events:none;}
  .fmu-detail-avatar{width:76px;height:76px;border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:1.8rem;font-weight:700;color:white;flex-shrink:0;box-shadow:0 8px 24px rgba(0,0,0,0.3);}
  ...
  .fmu-detail-btn{display:flex;align-items:center;gap:7px;padding:9px 16px;border-radius:var(--radius-sm);font-family:inherit;font-size:0.82rem;font-weight:600;cursor:pointer;border:1.5px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:rgba(255,255,255,0.8);transition:all .15s;}
  ...
  .fmu-access-card{display:flex;align-items:center;gap:18px;padding:18px 22px;background:linear-gradient(135deg,#0f172a,#1e293b);border-radius:var(--radius-sm);color:white;border:1px solid rgba(255,255,255,0.05);}
  ...
  @media(max-width:640px){.fmu-root{padding:16px;}.fmu-detail-body{grid-template-columns:1fr;}.fmu-info-card.full{grid-column:span 1;}.fmu-detail-hero{flex-wrap:wrap;}.fmu-grid{grid-template-columns:1fr;}}
`;
...
function InfoRow({ icon, label, value }) {
  return (
    <div className="fmu-info-row">
      <div className="fmu-info-row-icon">{icon}</div>
      <div><span className="fmu-info-label">{label}</span><span className="fmu-info-value">{value}</span></div>
    </div>
  );
}
...
function UserCard({ user, onView, onEdit, onDelete }) {
  ...
  return (
    <div className="fmu-card" style={{"--c1":c1,"--c2":c2}} onClick={() => onView(user)}>
      <div className="fmu-card-top">
        <div className="fmu-avatar" style={{background:`linear-gradient(135deg,${c1},${c2})`}}>{getInitials(user.name)}</div>
        <div className="fmu-card-actions">
          <button className="fmu-icon-btn" title="Edit" onClick={e=>{e.stopPropagation();onEdit(user);}}><FaEdit size={28}/></button>
          <button className="fmu-icon-btn danger" title="Delete" onClick={e=>{e.stopPropagation();onDelete(user.id);}}><FaTrash size={28}/></button>
        </div>
      </div>
...
function DetailView({ user, onBack, onEdit, onDelete }) {
  ...
  return (
    <>
      <button className="fmu-back-btn" onClick={onBack}><FaArrowLeft/> Back to users</button>
      <div className="fmu-detail-panel">
        <div className="fmu-detail-hero">
          <div className="fmu-detail-avatar" style={{background:`linear-gradient(135deg,${c1},${c2})`}}>{getInitials(user.name)}</div>
          <div className="fmu-detail-hero-info">
            ...
            <div className="fmu-detail-badges">
              ...
              <span className="fmu-detail-badge" style={isActive?{background:"rgba(16,185,129,0.25)",color:"#34d399"}:{background:"rgba(239,68,68,0.25)",color:"#fca5a5"}}>{user.status}</span>
            </div>
          </div>
        ...
        <div className="fmu-detail-body">
          <div className="fmu-info-card full">
            ...
            <div className="fmu-stats-row">
              {[[user.bookingsCount,"Total Bookings"],[fmtDate(user.lastActive),"Last Active"],[fmtDate(user.createdAt),"Member Since"]].map(([v,l])=>(
                <div className="fmu-stat-box" key={l}><div className="fmu-stat-box-val">{v}</div><div className="fmu-stat-box-label">{l}</div></div>
              ))}
            </div>
          </div>
          <div className="fmu-info-card">
            ...
            <div className="fmu-info-rows">
              ...
              <InfoRow icon={<MdBadge/>}     label="Username" value={`@${user.username}`}/>
            </div>
          </div>
          <div className="fmu-info-card">
            <p className="fmu-info-card-title">Employment</p>
            <div className="fmu-info-rows">
              <InfoRow icon={<MdBadge/>}        label="Matricule"   value={user.matricule}/>
              <InfoRow icon={<FaCalendarAlt/>}  label="Department"  value={user.department}/>
              <InfoRow icon={<FaCalendarAlt/>}  label="Position"    value={user.position}/>
              {user.site && <InfoRow icon={<FaMapMarkerAlt/>} label="Site" value={user.site}/>}
            </div>
          </div>
          {user.accessCard && (
            <div className="fmu-info-card full">
...
export default function FM_Users() {
  ...
  const filtered = users
    ...
    .sort((a, b) => {
      const cmp = sortBy === "name"     ? a.name.localeCompare(b.name)
                : sortBy === "bookings" ? a.bookingsCount - b.bookingsCount
  ...
  const handleAdd    = useCallback(u  => { setUsers(p => [...p, u]); setShowAddModal(false); }, []);
  const handleEdit   = useCallback(u  => { setUsers(p => p.map(x => x.id === u.id ? u : x)); setSelectedUser(u); setShowEditModal(false); }, []);
  ...
  const openDetail = u => { setSelectedUser(u); setView("detail"); };
  const openEdit   = u => { setSelectedUser(u); setShowEditModal(true); };

  const SELECTS = [
    [roleFilter,   setRoleFilter,   ROLES,                                          ],
    [deptFilter,   setDeptFilter,   DEPARTMENTS,                                    ],
    [statusFilter, setStatusFilter, ["All","active","inactive"], ["All Status","Active","Inactive"]],
  ];

  return (
    <div className="fmu-root">
      <div className="fmu-header">
        <div><h1 className="fmu-title">User Management</h1><p className="fmu-sub">Manage employees, roles & access cards</p></div>
        {view === "list" && <button className="fmu-add-btn" onClick={() => setShowAddModal(true)}><FaUserPlus/> Add User</button>}
      </div>
      ...
      {view === "list" ? (<>
        <div className="fmu-toolbar">
          <div className="fmu-search-wrap">
            ...
            <input className="fmu-search" placeholder="Search by name, email or matricule…" value={search} onChange={e => setSearch(e.target.value)}/>
          </div>
          {SELECTS.map(([val, set, opts, labels]) => (
            <div className="fmu-select-wrap" key={opts[0]}>
              <select className="fmu-select" value={val} onChange={e => set(e.target.value)}>
        ...
        <div className="fmu-grid">
          {filtered.length === 0
            ? <div className="fmu-empty"><div className="fmu-empty-icon">🔍</div><p>No users match your filters.</p></div>
            : filtered.map(u => <UserCard key={u.id} user={u} onView={openDetail} onEdit={openEdit} onDelete={handleDelete}/>)
          }
        </div>
      </>) : (
        selectedUser && <DetailView user={selectedUser} onBack={() => setView("list")} onEdit={() => openEdit(selectedUser)} onDelete={handleDelete}/>
      )}

      <AddUserModal isOpen={showAddModal}  onClose={() => setShowAddModal(false)}  onSave={handleAdd}  users={users}/>
      {selectedUser && <AddUserModal isOpen={showEditModal} onClose={() => setShowEditModal(false)} onSave={handleEdit} users={users} editUser={selectedUser}/>}
    </div>
  );
}